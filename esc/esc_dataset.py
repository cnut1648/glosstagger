        ...
        for data_path, keys_path in zip(self.data_paths, self.keys_paths):
            ...
            for _, _, wsd_sentence in raganato_iterable:
                ...
                for i, wsd_instance in enumerate(wsd_sentence):
                    ...
                    # encoder = <s>sent(with highlighted true label)</s></s>gloss</s> # gloss_pos = list of (start, pos)
...
def output_batch(current_batch: list, tokenizer) -> Dict[str, Any]:
    batched_sequences = batch_data([be.encoded_final_sequence for be in current_batch], tokenizer.pad_token_id)
    attention_masks = [torch.ones_like(be.encoded_final_sequence) for be in current_batch]
    batched_attention_masks = batch_data(attention_masks, pad_token_id=0)

    batch_dict = {
        "dataset_identifier": "wsd",
        "sequences": batched_sequences,
        "attention_masks": batched_attention_masks,
    }
    ...
    return batch_dict
def prepare_sent(sent: str, tokenizer, nlp):

    data_store = []
    tokens = []
    sentence_tokens = list(map(str, nlp(sent)))
    for i, token in enumerate(nlp(sent)):
        if token.pos_ not in ["VERB", "NOUN"]:
            continue
        possible_offsets = wn_offsets_from_lemmapos(
            token.lemma_, pos_map.get(token.pos_, token.pos_)
        )
        curr_sentence = tokenizer.insert_classify_tokens(sentence_tokens, index=i)
        possible_glosses = [
            synset_from_offset(po).definition().capitalize() + "." for po in possible_offsets
        ]

        encoded_final_sequence, gloss_positions, token_type_ids = tokenizer.prepare_sample(
            curr_sentence, possible_glosses
        )
        start_position, end_position = None, None
        ...
        data_store.append(data_elem)
        tokens.append((token.text, {
            "lemma": token.lemma_,
            "pos": token.pos_,
        }))
    return output_batch(data_store, tokenizer), tokens
