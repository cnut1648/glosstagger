...
class ESCModule(pl.LightningModule):
    def __init__(self, conf, *args, **kwargs):
        ...
        self.hp = self.hparams['conf']
        if any([tm in self.hp["transformer_model"] for tm in SUPPORTED_MODELS]):
            if self.hp["squad_head"]:
                self.qa_model = SquadQAModel(self.hp)
            elif "xlnet" in self.hp["transformer_model"]:
                self.qa_model = WSDXLNetForQuestionAnswering.from_pretrained(
                    self.hp["transformer_model"], mem_len=1024
                )
            else:
                self.qa_model = AutoModelForQuestionAnswering.from_pretrained(self.hp["transformer_model"])
        else:
            raise NotImplementedError

        if getattr(self.hp, "use_special_tokens", False):
            self.qa_model.resize_token_embeddings(self.hp["vocab_size"])
    ...
    def forward(
        ...
        if "bart" not in self.hp["transformer_model"]:
            model_input["token_type_ids"] = kwargs.get("token_type_ids", None)

        if self.hp["squad_head"] and self.hp["use_pmask"]:
    ...
    def validation_step(self, batch, batch_idx: int, *args, **kwargs):
        ...
        loss = forward_output["loss"]
        self.log(f'{batch["dataset_identifier"]}_val_loss', loss)
    ...
    def validation_epoch_end(self, all_outputs):
        ...
        for i, outputs in enumerate(all_outputs):
            ...
            self.log(f"{prefix}_correct_end_predictions", torch.sum(correct_end_predictions) / predictions_len)
            ...
            self.log(f"{prefix}_in_bound_predictions", torch.sum(in_bound_predictions) / predictions_len)
    ...
    def get_optimizer_and_scheduler(self):
        ...
        no_decay = self.hp["no_decay_params"]
        ...
        if self.hp["optimizer"] == "adam":
            optimizer = torch.optim.AdamW(optimizer_grouped_parameters, self.hp["learning_rate"])
        elif self.hp["optimizer"] == "lamb":
            optimizer = Lamb(optimizer_grouped_parameters, self.hp["learning_rate"])
        elif self.hp["optimizer"]== "radam":
            optimizer = RAdam(optimizer_grouped_parameters, self.hp["learning_rate"])
        ...
        lr_scheduler = get_linear_schedule_with_warmup(
            optimizer=optimizer,
            num_warmup_steps=self.hp["num_warmup_steps"],
            num_training_steps=self.hp["num_training_steps"],
        )
