...
def predict(
    ...
    with torch.no_grad(), torch.cuda.amp.autocast():
        ...
        for batch in data_loader:
            ...
            if device >= 0:
                batch["sequences"] = model.transfer_batch_to_device(batch["sequences"], model.device, 0)
                batch["attention_masks"] = model.transfer_batch_to_device(batch["attention_masks"], model.device, 0)
            ...
            for i, seq in enumerate(batch["sequences"]):
                ...
                if "xlnet" in model.hp["transformer_model"] or model.hp["squad_head"]:
                    ei = ei[0]
...
def main() -> None:
    ...
    known_raganato_paths = list_elems_in_dir("/home/jiashu/WSD/esc/data/WSD_Evaluation_Framework/Evaluation_Datasets", only_dirs=True)
    ...
    tokenizer = get_tokenizer(
        wsd_model.hp["transformer_model"], getattr(wsd_model.hp, "use_special_tokens", False)
    )
...
def predict_one_batch(model, batch):
    batch["sequences"] = model.transfer_batch_to_device(batch["sequences"], model.device, 0)
    batch["attention_masks"] = model.transfer_batch_to_device(batch["attention_masks"], model.device, 0)

    predictions = model(batch["sequences"], batch["attention_masks"])
    predicted_offsets_list = []
    for i, seq in enumerate(batch["sequences"]):
        seq = seq.cpu().numpy().tolist()
        ...
        predicted_offsets = probabilistic_prediction(
            start_logits_i, end_logits_i, glosses_indices, possible_offsets, "probabilistic"
        )

        predicted_offsets_indices = [glosses_indices[possible_offsets.index(po)] for po in predicted_offsets]

        predicted_offsets_list.append(predicted_offsets[0])

    return predicted_offsets_list 
