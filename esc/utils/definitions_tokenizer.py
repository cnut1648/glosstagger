...
class DefinitionsTokenizer:
    ...
    def encode_pair(
        ...
        encoding_out = self.tokenizer(sequence1, sequence2, return_tensors="pt", return_token_type_ids=True)
    ...
    def prepare_sample_without_st(
        ...
        for gsi, gei in definitions_offsets:
            ranges = []
            for i, (si, ei) in enumerate(offsets):
                if gsi <= si <= ei <= gei:
                    ranges.append(i)
            definitions_positions.append((context_offset + min(ranges), context_offset + max(ranges)))
